...
class AssetController {
    ...
    public function getAllAssets() {
        try {
            ...
            $has_warranty = isset($_GET['has_warranty']) ? (int)$_GET['has_warranty'] : 0;
    ...
    public function getAssetById($id) {
        try {
            ...
            // ດຶງຂໍ້ມູນເພີ່ມເຕີມ
            $asset['documents'] = $this->documentModel->getByAssetId($id);
            $asset['images'] = $this->imageModel->getByAssetId($id);
            
            // ດຶງຄຸນສົມບັດຂອງໝວດໝູ່
            if ($asset['category_id']) {
                $asset['category_attributes'] = $this->categoryAttributeModel->getByCategoryId($asset['category_id']);
                $asset['category_inheritance'] = $this->categoryInheritanceModel->getByCategoryId($asset['category_id']);
            }
    ...
    public function createAsset() {
        try {
            ...
            // ກວດສອບສິດ - ສະເພາະ super_admin ແລະ asset_admin
            ...
            // ກວດສອບຂໍ້ມູນ (ຫຍໍ້ລົງເພື່ອຄວາມກະທັດຮັດ)
            $validationRules = [
                'asset_name' => 'required|string|max:255',
                'asset_name_en' => 'string|max:255',
                'category_id' => 'required|integer',
                'company_id' => 'required|integer',
                'department_id' => 'required|integer',
                'status' => 'in:available,in_use,maintenance,disposed,lost,reserved',
                'asset_condition' => 'in:new,good,fair,poor,damaged,obsolete'
            ];
            ...
            if ($result['success']) {
                // ບັນທຶກການສ້າງ
                $this->userModel->logActivity($currentUser['id'], 'asset_created', 
                    'Asset created: ' . $data['asset_name']);
    ...
    public function uploadImage($assetId) {
        try {
            ...
            $result = $this->imageModel->create($data, $currentUser['id']);
