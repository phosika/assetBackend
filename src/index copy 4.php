...
require_once __DIR__ . '/controllers/InventoryItemController.php'; // ເພີ່ມ InventoryItemController
// require_once __DIR__ . '/controllers/InventoryController.php'; // ເພີ່ມ InventoryController
...
// require_once __DIR__ . '/models/Product.php';                   // ເພີ່ມ Product model
...
// require_once __DIR__ . '/models/Inventory.php';                  // ເພີ່ມ Inventory model
...
try {
    switch ($resource) {
        ...
        case 'inventory':
            ...
            // GET /inventory - ດຶງຂໍ້ມູນສິນຄ້າຄົງຄັງທັງໝົດ
            if ($method === 'GET' && $action === '' && $id === '') {
                $inventoryController->getAllInventory();
            }
            // GET /inventory/low-stock - ດຶງສິນຄ້າທີ່ໃກ້ໝົດສະຕ໋ອກ
            elseif ($method === 'GET' && $action === 'low-stock') {
                $inventoryController->getLowStockItems();
            }
            // GET /inventory/out-of-stock - ດຶງສິນຄ້າທີ່ໝົດສະຕ໋ອກ
            elseif ($method === 'GET' && $action === 'out-of-stock') {
                $inventoryController->getOutOfStockItems();
            }
            // GET /inventory/stats - ດຶງສະຖິຕິສິນຄ້າ
            elseif ($method === 'GET' && $action === 'stats') {
                $inventoryController->getInventoryStats();
            }
            // GET /inventory/movements - ດຶງປະຫວັດການເຄື່ອນໄຫວສິນຄ້າ
            elseif ($method === 'GET' && $action === 'movements') {
                $inventoryController->getInventoryMovements();
            }
            // GET /inventory/by-product/{productId} - ດຶງສິນຄ້າຕາມ product ID
            elseif ($method === 'GET' && $action === 'by-product' && !empty($id)) {
                $inventoryController->getInventoryByProduct($id);
            }
            // GET /inventory/by-location/{locationId} - ດຶງສິນຄ້າຕາມສະຖານທີ່
            elseif ($method === 'GET' && $action === 'by-location' && !empty($id)) {
                $inventoryController->getInventoryByLocation($id);
            }
            // GET /inventory/{id} - ດຶງສິນຄ້າຕາມ ID
            elseif ($method === 'GET' && !empty($action) && is_numeric($action)) {
                $inventoryController->getInventoryById($action);
            }
            // POST /inventory/adjust - ປັບປຸງຈຳນວນສິນຄ້າ
            elseif ($method === 'POST' && $action === 'adjust') {
                $inventoryController->adjustInventory();
            }
            // POST /inventory/transfer - ໂອນສິນຄ້າລະຫວ່າງສະຖານທີ່
            elseif ($method === 'POST' && $action === 'transfer') {
                $inventoryController->transferInventory();
            }
            // POST /inventory/count - ບັນທຶກການນັບສິນຄ້າ
            elseif ($method === 'POST' && $action === 'count') {
        ...
        case 'products':
            ...
            // GET /products - ດຶງຂໍ້ມູນສິນຄ້າທັງໝົດ
            if ($method === 'GET' && $action === '' && $id === '') {
                $inventoryController->getAllProducts();
            }
            // GET /products/dropdown - ດຶງຂໍ້ມູນສິນຄ້າສຳລັບ dropdown
            elseif ($method === 'GET' && $action === 'dropdown') {
                $inventoryController->getProductsForDropdown();
            }
            // GET /products/categories - ດຶງໝວດໝູ່ສິນຄ້າ
            elseif ($method === 'GET' && $action === 'categories') {
                $inventoryController->getProductCategories();
            }
            // GET /products/search - ຄົ້ນຫາສິນຄ້າ
            elseif ($method === 'GET' && $action === 'search') {
                $inventoryController->searchProducts();
            }
            // GET /products/by-category/{categoryId} - ດຶງສິນຄ້າຕາມໝວດໝູ່
            elseif ($method === 'GET' && $action === 'by-category' && !empty($id)) {
                $inventoryController->getProductsByCategory($id);
            }
            // GET /products/{id} - ດຶງສິນຄ້າຕາມ ID
            elseif ($method === 'GET' && !empty($action) && is_numeric($action)) {
                $inventoryController->getProductById($action);
            }
            ...
            elseif ($method === 'PUT' && !empty($action) && is_numeric($action)) {
                $inventoryController->updateProduct($action);
            }
            // DELETE /products/{id} - ລຶບສິນຄ້າ
            elseif ($method === 'DELETE' && !empty($action) && is_numeric($action)) {
                $inventoryController->deleteProduct($action);
            }
            // POST /products/{id}/image - ອັບໂຫຼດຮູບສິນຄ້າ
            elseif ($method === 'POST' && !empty($action) && is_numeric($action) && $id === 'image') {
                $inventoryController->uploadProductImage($action);
            }
            else {
                Response::notFound('Products endpoint not found');
            }
            break;
        ...
        case 'purchases':
            ...
            // GET /purchases - ດຶງໃບສັ່ງຊື້ທັງໝົດ
            if ($method === 'GET' && $action === '' && $id === '') {
                $purchaseController->getAllPurchaseOrders();
            }
            // GET /purchases/stats - ດຶງສະຖິຕິໃບສັ່ງຊື້
            elseif ($method === 'GET' && $action === 'stats') {
                $purchaseController->getPurchaseStats();
            }
            // GET /purchases/pending - ດຶງໃບສັ່ງຊື້ທີ່ຍັງບໍ່ທັນສຳເລັດ
            elseif ($method === 'GET' && $action === 'pending') {
                $purchaseController->getPendingOrders();
            }
            // GET /purchases/by-supplier/{supplierId} - ດຶງໃບສັ່ງຊື້ຕາມຜູ້ສະໜອງ
            elseif ($method === 'GET' && $action === 'by-supplier' && !empty($id)) {
                $purchaseController->getPurchaseOrdersBySupplier($id);
            }
            // GET /purchases/{id} - ດຶງໃບສັ່ງຊື້ຕາມ ID
            elseif ($method === 'GET' && !empty($action) && is_numeric($action)) {
            ...
            // PATCH /purchases/{id}/status - ອັບເດດສະຖານະໃບສັ່ງຊື້
            elseif ($method === 'PATCH' && !empty($action) && is_numeric($action) && $id === 'status') {
        ...
        case 'sales':
            ...
            // GET /sales/today - ດຶງຍອດຂາຍມື້ນີ້
            elseif ($method === 'GET' && $action === 'today') {
                $salesController->getTodaySales();
            }
            // GET /sales/by-customer/{customerId} - ດຶງໃບຂາຍຕາມລູກຄ້າ
            elseif ($method === 'GET' && $action === 'by-customer' && !empty($id)) {
                $salesController->getSalesByCustomer($id);
            }
            ...
            // PUT /sales/{id} - ອັບເດດໃບຂາຍ
            elseif ($method === 'PUT' && !empty($action) && is_numeric($action)) {
                $salesController->updateSalesOrder($action);
            }
            // PATCH /sales/{id}/status - ອັບເດດສະຖານະໃບຂາຍ
            elseif ($method === 'PATCH' && !empty($action) && is_numeric($action) && $id === 'status') {
                $salesController->updateSalesStatus($action);
            }
            // DELETE /sales/{id} - ລຶບໃບຂາຍ
            elseif ($method === 'DELETE' && !empty($action) && is_numeric($action)) {
        ...
        case 'suppliers':
            ...
            // GET /suppliers/by-code/{code}
            elseif ($method === 'GET' && $action === 'by-code' && !empty($id)) {
                $supplierController->getSupplierByCode($id);
            }
            // GET /suppliers/{id}
            elseif ($method === 'GET' && !empty($action) && is_numeric($action)) {
            ...
            elseif ($method === 'PATCH' && !empty($action) && is_numeric($action) && $id === 'status') {
                $supplierController->updateSupplierStatus($action);
            }
