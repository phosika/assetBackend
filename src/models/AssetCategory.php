...
public function getAllCategories($filters = [], $userRole = null) {
    ...
    // ກັ່ນຕອງຕາມວິທີເສື່ອມລາຄາ
    if (!empty($filters['depreciation_method']) && $filters['depreciation_method'] !== 'all') {
        $sql .= " AND ac.depreciation_method = ?";
        $params[] = $filters['depreciation_method'];
    }
    ...
    // ເພີ່ມ parent_display ສຳລັບແຕ່ລະລາຍການ
    foreach ($categories as &$category) {
        if ($category['parent_id']) {
            $category['parent_display'] = $category['parent_category_name'] . ' (' . $category['parent_category_code'] . ')';
        } else {
            $category['parent_display'] = null;
        }
    }
    ...
    if (!empty($filters['depreciation_method']) && $filters['depreciation_method'] !== 'all') {
        $countSql .= " AND ac.depreciation_method = ?";
        $countParams[] = $filters['depreciation_method'];
    }
    ...
    public function getById($id) {
        ...
        if ($category) {
            // ສ້າງ display name ສຳລັບ parent
            if ($category['parent_id']) {
                $category['parent_display'] = $category['parent_category_name'] . ' (' . $category['parent_category_code'] . ')';
            } else {
                $category['parent_display'] = null;
    ...
    public function create($data, $createdBy) {
        ...
        $level = $data['level'] ?? 1; // ຮັບຄ່າ level ຈາກ Frontend
    ...
    public function update($id, $data) {
        ...
        try {
            ...
            // ອັບເດດລູກຫຼານທັງໝົດ (ຖ້າມີການປ່ຽນ path)
            if ($needsPathUpdate) {
    ...
    /**
     * ດຶງຂໍ້ມູນໝວດໝູ່ຊັບສິນຕາມລະດັບສູງສຸດ
     */
    public function getCategoriesByMaxLevel($maxLevel, $excludeId = null) {
