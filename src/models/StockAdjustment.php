...
class StockAdjustment {
    ...
    public function getAllAdjustments($filters = [], $page = 1, $limit = 20) {
        try {
            ...
            error_log("=== getAllAdjustments ===");
            error_log("Total records: " . $total);
            error_log("Returned records: " . count($adjustments));
    ...
    public function createAdjustment($data, $createdBy) {
        try {
            ...
            error_log("Generated code: " . $adjustmentCode);
            ...
            if ($result) {
                ...
                // ປັບສະຕ໋ອກທັນທີ
                $applyResult = $this->applyAdjustment($adjustmentId);
                error_log("Apply adjustment result: " . ($applyResult ? 'success' : 'failed'));
    ...
    private function applyAdjustment($adjustmentId) {
        try {
            error_log("=== applyAdjustment ===");
            error_log("Adjustment ID: " . $adjustmentId);
            ...
            if (!$adjustment) {
                error_log("Adjustment not found: $adjustmentId");
                return false;
            }
            
            error_log("Adjustment data: " . json_encode($adjustment));
            ...
            error_log("Item: $itemId, Warehouse: " . ($warehouseId ?? 'NULL') . ", Quantity: $quantity, Type: $type");
            ...
            error_log("Old quantity: $oldQuantity");
            ...
            $sql = "UPDATE {$this->table} 
                    ...
                    WHERE id = ?";
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$adjustment['created_by'], $adjustmentId]);
            
            error_log("Applied adjustment $adjustmentId successfully");
            return true;
    ...
    private function generateAdjustmentCode() {
        ...
        try {
            $stmt = $this->db->prepare("SELECT COUNT(*) as count FROM {$this->table} WHERE adjustment_code LIKE ?");
            $stmt->execute(["{$prefix}-{$year}{$month}%"]);
            $count = $stmt->fetch()['count'] + 1;
            
            $code = $prefix . '-' . $year . $month . str_pad($count, 4, '0', STR_PAD_LEFT);
            error_log("Generated adjustment code: $code");
            return $code;
            
        } catch (Exception $e) {
            error_log("Error generating code: " . $e->getMessage());
            // Fallback ຖ້າມີ error
